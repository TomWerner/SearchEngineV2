...
public class ComplexBadTokenParser implements TokenParser
{
    public ComplexBadTokenParser()
    {
    }
     ...
     * 
     * @see
     * org.uiowa.cs2820.engine.queryparser.TokenParser#parseTokens(java.util
     * .ArrayList)
     */
    @Override
    public Queryable parseTokens(ArrayList<Token> tokens) throws ParsingException
    {
        ...
        try
        {
            ...
            if (tokens.get(index).getType() != Token.QUERY_START) // Only one
                                                                  // query
            {
            ...
            else
            {
                ...
                // There are still tokens left, -2 b/c we can ignore the last
                // paren
                while (index < tokens.size() - 2)
                {
                    ...
                    while (index < tokens.size() - 1 && tokenFound(Token.PAREN_END, index, tokens))
                    {
                        queries.add(tokens.get(index));
                        index++;
                    }
    ...
    protected Queryable parseQueryList(ArrayList<Object> queries)
    {
        ...
        // new list which is then converted to a double query and stuck back on
        // the stack
        ...
        while (index < queries.size())
        {
            ...
            Queryable q1 = (Queryable) subStack.pop();
            QueryOperator qOp = (QueryOperator) subStack.pop();
            Queryable q2 = (Queryable) subStack.pop();
            query = new DoubleQuery(q1, q2, qOp);
            ...
            index++; // Get past the previous right paren
        }
